...
import { User, Mail, Lock, Phone, Briefcase, ArrowRight, Sparkles, ShieldCheck, ChevronDown } from 'lucide-react';
      ...
      <div className="flex min-h-screen">
        ...
        <div className="glass-sidebar">
          <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="space-y-6">
            ...
            <h1 className="text-4xl font-black tracking-tighter">ServiceMate</h1>
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md space-y-8">
              ...
              {['customer', 'provider'].map((r) => (
                <button
                  ...
                  className={`flex-1 py-3 rounded-xl font-bold transition-all capitalize ${
                    form.role === r ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-500 hover:text-white'
                  }`}
                >
                  {r}
                </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {form.role === 'provider' && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
                    <div className="premium-input-wrapper group">
                      ...
                      <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 group-hover:text-indigo-400 pointer-events-none" size={20} />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="premium-input-wrapper group">
                ...
                <input className="premium-input" placeholder="Phone Number" onChange={(e) => setForm({ ...form, phone: e.target.value })} />
              </div>
